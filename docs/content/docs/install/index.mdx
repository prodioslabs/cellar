...
Tag `vX.Y.Z` (or a SemVer prerelease such as `vX.Y.Z-rc.1`) to publish Linux **amd64** and **arm64** bundles. macOS and Windows are not supported release targets — Cellar’s runtime depends on Linux Docker, gVisor `runsc`, and iptables.
...
Each tarball co-locates all four binaries (so `cellard` can find `cellar-agent` beside itself) plus `LICENSE`, `README.md`, and `contrib/systemd/`. Verify the archive against `checksums.txt` before installing.

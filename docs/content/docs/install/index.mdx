...
Requires Go 1.26+. `cellar-agent` and `cellar-egress-gateway` are built with `CGO_ENABLED=0`. `make install` places them next to `cellard` (override with `CELLAR_AGENT_BINARY` if needed). It also installs:

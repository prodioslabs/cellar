...
Network limits accept structured `mode`/`rules` or Daytona-style sugar on `NetworkPolicy`:
`networkAllowList` (IPv4 CIDRs), `domainAllowList` (domains / `*.` wildcards), `blockAll`,
and opt-in `essentialServices`. At most one sugar field may be set.

---
title: TypeScript client
description: '@cellar/node SDK for Node.js 18+ and Bun'
---

Package: [`@cellar/node`](https://github.com/prodioslabs/cellar/tree/main/sdk/node). Works on **Node.js 18+** and **Bun**. Talks to `cellar-gateway` over HTTPS with an API key — same model as the [Go client](/docs/sdk/go).

## Install

```bash
npm install @cellar/node
# or: bun add @cellar/node
```

## Configure

| Variable          | Required | Meaning                                           |
| ----------------- | -------- | ------------------------------------------------- |
| `CELLAR_API_KEY`  | yes      | Raw key from `cellar api-key create` (`cellar_…`) |
| `CELLAR_ENDPOINT` | yes      | Gateway base URL (`https://cellar.example.com`)   |

```bash
export CELLAR_API_KEY='cellar_…'
export CELLAR_ENDPOINT='https://cellar.example.com'
```

## Usage

```ts
import { Client } from '@cellar/node'

const c = Client.fromEnv()
// or: Client.create({ endpoint: 'https://cellar.example.com', apiKey: 'cellar_…' })

const sb = await c.create({
  spec: { image: 'alpine:3.20' },
})
console.log('created', sb.id)

// Creation returns immediately (often pending). Wait until the container is running.
await sb.waitUntilReady()

const res = await sb.exec(['uname', '-a'])
console.log(`exit=${res.exitCode} stdout=${res.stdout.toString()}`)

await sb.fs.write('/tmp/hello.txt', 'hi')
console.log(await sb.fs.readToString('/tmp/hello.txt'))
console.log(await sb.fs.stat('/tmp/hello.txt'))

for await (const chunk of sb.logs({ tail: 100 })) {
  process.stdout.write(chunk.data)
}

await sb.remove()
```

`Client` ops: `create`, `get`, `list`.

`Sandbox` ops: `waitUntilReady`, `getStatus`, `exec`, `startJob`, `listJobs`, `getJob`, `stopJob`, `logs`, `stop`, `remove`, `updateNetwork`, and `fs` (read/write/stat/list/mkdir/remove/removeDir/exists/copy/rename, plus `readStream`/`writeStream`).

Paths for `fs` must be absolute. `mkdir` / `write` do not create parent directories. `removeDir` is non-recursive (empty dirs only).

Network limits on `NetworkPolicy`: `networkAllowList`, `domainAllowList`, `blockAll`,
and opt-in `essentialServices`. At most one of the first three may be set; they
translate to canonical `mode`/`rules`/`dns` server-side.

Creation is asynchronous with respect to runtime readiness. Prefer `await sb.waitUntilReady()` before `exec` or `fs`. Status is refreshed from cellar-gateway via `GET /v1/sandboxes/:id`.

Auth is sent as `Authorization: Bearer …` and `X-Api-Key`.

## Develop

From `sdk/node` (requires [Bun](https://bun.sh)):

```bash
bun install
bun run test
bun run build    # or: make sdk-node (from repo root)
```

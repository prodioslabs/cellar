...
# Filesystem (absolute guest paths; mkdir/write do not create parents; remove-dir is non-recursive):
echo hello | sudo cellar sandbox fs write <id> /tmp/hello.txt
sudo cellar sandbox fs read <id> /tmp/hello.txt
sudo cellar sandbox fs stat <id> /tmp/hello.txt
sudo cellar sandbox fs list <id> /tmp
sudo cellar sandbox stop <id>
sudo cellar sandbox rm <id>
```

**Filesystem.** Guest file I/O is implemented by `cellar-agent fs …` inside the container (same docker-exec path as background jobs). SDKs expose `sandbox.fs` / `Sandbox.FS()` against the gateway HTTP routes under `/v1/sandboxes/:id/fs/…`.

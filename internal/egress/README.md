...
destinations on the public internet. When a sandbox _does_ need network
...
Each sandbox sits alone on a tiny private Docker network marked `Internal: true`. That flag means Docker will not give the network a route to the host
...
bridge (`cellar-egress`) that _can_ reach the outside world.
...
| 5    | **gRPC `RegisterSandbox`** on the gateway control port (policy + IPs)                                                                                        | Gateway must know the sandbox _before_ traffic can arrive                                                      |
...
Example: the policy is `domain_allow_list: google.com` and the guest runs
`curl https://google.com`.

1. curl asks DNS for `google.com`. The query goes to `.2` (forced via the
   bind-mounted `resolv.conf`).
2. The gateway evaluates DNS policy (`AllowDNS`):

- **Allowed** → answer with **the gateway’s own `.2` IP** (TTL 10s). Never
  a real upstream A record.
- **Denied** → **NXDOMAIN**, so the guest fails fast.

3. curl connects to that bait IP on `:443`. No iptables is involved for
   80/443 — the gateway _is_ the destination and listens there directly.
4. The gateway accepts, peeks the TLS ClientHello for SNI (`PeekTLSSNI`;
   `PeekHTTPHost` on `:80`) without consuming bytes, and re-evaluates
   connect policy against that name. The SNI check is what binds the
   connection to a domain — the bait DNS answer proves nothing, since
   every allowed lookup returns the same `.2`. Missing SNI is a deny.
5. On allow, the gateway re-resolves `google.com` itself and dials the
   real upstream **by hostname**, then splices the streams. TLS stays
   end-to-end — curl sees Google’s real certificate; nothing is
   terminated or served by the gateway.

Returning the gateway’s own IP is intentional: the sandbox thinks it is
talking to `google.com`, but every byte still flows through policy. And
dialing by name (instead of trusting anything the guest resolved) means
an `/etc/hosts` edit inside the guest cannot point an allowed name at an
internal address.
...
Example: the policy is `network_allow_list: 1.1.1.1/32` and the guest
dials `1.1.1.1:2345`.

1. No DNS happens — the guest dials the raw IP directly. Because of the
   route helper, the default route is via `.2`, so the packet arrives at
   the gateway with the original destination preserved.
2. iptables REDIRECT (`-s <sandbox .3> ! -d <gateway .2>`) sends it to
   the gateway’s catch-all listener on `:15000`.
3. The gateway recovers `1.1.1.1:2345` via `SO_ORIGINAL_DST` and
   evaluates policy with an **empty hostname** — the catch-all listener
   never peeks for SNI/Host (peeking would stall server-first protocols
   like SSH or Postgres). Only IP/CIDR rules can match here; domain
   rules never apply. `1.1.1.1` matches `1.1.1.1/32` → allow (port 2345
   passes because the rule lists no ports).
4. Since the match was CIDR, the gateway dials the **original ip:port**
   as-is — no re-resolution, because no name was ever asserted — and
   splices.
